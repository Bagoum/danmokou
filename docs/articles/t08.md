...
In almost all cases, you will want to record the two functions `loc` (location) and `dir` (direction), defined in <xref:DMath.SBV2Repo>. We assign them to the aliases `eloc` and `edir` respectively. 
