...
`nrvelocity` (**N**on**R**otational **Velocity**) works similarly to `rvelocity`. What do you think would happen if you ran
...
- GIRepeat (Generalized IEnumerator Repeat) (type: <xref:DMK.Danmaku.Patterns.AsyncPatterns>) is an AsyncPattern, so it can use waits and delays. Its children must be AsyncPatterns.
- GCRepeat (Generalized Coroutine Repeat) (type: <xref:DMK.Danmaku.Patterns.AsyncPatterns>) is an AsyncPattern, so it can use waits and delays. Its children must be **SyncPatterns**. 
- GSRepeat (Generalized Synchronous Repeat) (type: <xref:DMK.Danmaku.Patterns.SyncPatterns>) is a SyncPattern. Its children must be SyncPatterns.
...
- an array of repeater modifiers (<xref:DMK.Danmaku.Options.GenCtxProperty>),
...
Before we show GTRepeat, let's take a look at the `move` State Machine, defined in <xref:DMK.SM.SMReflection>. Replace your firing code with this:
