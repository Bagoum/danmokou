...
Whenever we need to do something multiple times, we use a repeater. There are four repeaters, and they are all almost the same.

- GSRepeat (Generalized Synchronous Repeat)
- GCRepeat (Generalized Coroutine (Asynchronous) Repeat)
- GIRepeat (Generalized IEnumerator (Asynchronous) Repeat)
- GTRepeat (Generalized Task Repeat)
...
Before we show GTRepeat, let's take a look at the `move` SM function. Replace your firing code with this:
