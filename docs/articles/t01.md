...
The way to read this line is as follows: "fire a fireball, in red/w color, starting at the position (2, 0), with velocity (0, -2)". The screen is 16 units wide and 9 units high. The playable area goes from X=[-5, 5] and Y=[-4.5, 4].
...
- GSRepeat (Generalized Synchronous Repeat) (type: <xref:Danmaku.SyncPatterns>)
- GCRepeat (Generalized Coroutine (Asynchronous) Repeat) (type: <xref:Danmaku.AsyncPatterns>)
- GIRepeat (Generalized IEnumerator (Asynchronous) Repeat) (type: <xref:Danmaku.AsyncPatterns>)
