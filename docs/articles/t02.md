...
You can view all bullet controls here: @Danmaku.BulletManager.SimpleBulletControls, or in Danmaku/BulletManagement/BulletControls.cs:SimpleBulletControls.
...
However, we don't want to destroy the control immediately. Instead, we use the delay command `_(delaySeconds, subCommand)` to destroy it after four seconds.
...
To delete the control after four seconds, we use a predicate that returns true if the timer's elapsed time, in seconds, is less than 4. (Since BDSM is prefix-based, `timersec(pt) < 4` is written as `< timersec(pt) 4`.)
...
We're also now using `simple` instead of `s`. This command allows us to provide an array of simple bullet options (see <xref:Danmaku.SBOption> or Danmaku/BulletManagement/SimpleBulletOptions.cs), which are used for fancy effects. In our case, we want the stars to rotate independently of their movement, so we provide a custom direction function in degrees. The function `* 200 t` makes the bullets rotate 200 degrees counterclockwise per second.
