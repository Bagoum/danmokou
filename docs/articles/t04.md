...
- Generic summonable options (<xref:Danmaku.BehOption> or BulletManagement/BehOptions.cs)

We'll discuss the summonable options more later in this tutorial, but for now let's appreciate the vanity of the `hueshift` option. The hue spectrum is 360 degrees, and the `hueshift(X)` option makes the rendered data shift its hue by `X * T` degrees where T is the lifetime of the object.
...
- A list of LaserOptions (<xref:Danmaku.LaserOption> or BulletManagement/LaserOptions.cs)
		...
		sync gdlaser-green/b <;3:1;:270> gsr2 6 <24> {
			...
			bindItr(itr)
				...
				1 12 {
					...
					start(+ 2 sine 3h 2 t)
					s(2)
					dynamic(roffset(pxy(* 0.5 t,  sine 5 0.2 (+ * 2.5 &itr + t * 8 &lt))))
				}
```
...
`icrow` is a *summonable*. The list of summonables is at Assets/SO/References/Default Summonable Styles. Enemies are all summables, although bosses are usually not handled via summoning. 
...
If a summonable is not an enemy, your bullets will not damage it or interact with it. 

We introduce the modifier `target` here. This runs at the start of the repeater and modifies the V2RV2 based on a target location. The most common method is Angle (see <xref:Danmaku.Enums.RV2ControlMethod>), which rotates the V2RV2 to point at the target location. Lplayer is the location of the player. As a result, even though the original V2RV2 has an angle of zero, the first crow actually fires towards the player.
...
**Warning: If you want to spawn a summon without a state machine, do NOT use the state machine `null`. Instead, use the state machine `wait-phase`. The difference is that summons with state machine `wait-phase` will  automatically get cleaned up when the summoner ends the phase, but summons will state machine `null` will persist until explicitly destroyed via a Cull command.**
