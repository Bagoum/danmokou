...
- Generic summonable options (<xref:DMK.Danmaku.Options.BehOption> or BulletManagement/BehOptions.cs)
...
- A list of LaserOptions (<xref:DMK.Danmaku.Options.LaserOption> or BulletManagement/LaserOptions.cs)
...
We introduce the modifier `target` here. This runs at the start of the repeater and modifies the V2RV2 based on a target location. The most common method is Angle (see <xref:DMK.Core.RV2ControlMethod>), which rotates the V2RV2 to point at the target location. Lplayer is the location of the player. As a result, even though the original V2RV2 has an angle of zero, the first crow actually fires towards the player.
