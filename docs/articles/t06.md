...
- Add a simple bullet modifier (<xref:Danmaku.SBOption>) like `Opacity(Func<SimpleBullet, float> opacityMultiplier)`, and add support in the engine for setting `SimpleBullet.opacityMultiplier` when the modifier is present.
