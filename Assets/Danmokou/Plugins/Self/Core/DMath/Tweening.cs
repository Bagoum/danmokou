...
public static class Tweening {
    public static readonly Func<float, float> Identity = x => x;
    private static IEnumerator Ease<T>(T start, T end, Action<T> apply, 
        Func<T, T, float, T> lerp, float time, Func<float, float>? ease = null, ICancellee? cT = null) {
    ...
    private static IEnumerator Ease(float start, float end, Action<float> apply, float time,
        Func<float, float>? ease = null, ICancellee? cT = null) =>
        Ease(start, end, apply, Mathf.Lerp, time, ease, cT);
    private static IEnumerator Ease(Vector3 start, Vector3 end, Action<Vector3> apply, float time,
        Func<float, float>? ease = null, ICancellee? cT = null) =>
        Ease(start, end, apply, Vector3.Lerp, time, ease, cT);
    ...
    public static IEnumerator ColorTo(this SpriteRenderer sr, Color target, float time, 
        Func<float, float>? ease = null, ICancellee? cT = null)
        => Ease(sr.color, target, c => sr.color = c, Color.Lerp, time, ease, cT);
