...
public static class FSInterop {
    public static T[] ToNullableArray<T>(IEnumerable<FSharpOption<T>> lt) where T : class =>
        lt.Select(x => x.Nullable()).ToArray();

    public static FSharpMap<T, V> NewMap<T, V>() => new FSharpMap<T, V>(new Tuple<T, V>[] { });

    public static FSharpMap<T, V> AsMap<T, V>(Dictionary<T, V> dict) =>
        new FSharpMap<T, V>(dict.Keys.Select(x => new Tuple<T, V>(x, dict[x])));
