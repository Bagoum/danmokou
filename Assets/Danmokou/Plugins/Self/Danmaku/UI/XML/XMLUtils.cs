...
public static class XMLUtils {
    ...
    public static UIScreen ReplayScreen(Action<List<int>> cacheTentative, Action cacheConfirm) =>
        new UIScreen(SaveData.p.ReplayData.Count.Range().Select(i =>
            new CacheNavigateUINode(cacheTentative, () =>
                    SaveData.p.ReplayData.TryN(i)?.metadata.Record.AsDisplay(true, true) ?? "---Deleted Replay---",
                new FuncNode(() => {
                    cacheConfirm();
    ...
    public static UIScreen HighScoreScreen(VisualTreeAsset optionNode, UIScreen replayScreen,
        SMAnalysis.AnalyzedCampaign[] campaigns, [CanBeNull] SMAnalysis.AnalyzedDayCampaign days = null) {
        ...
        var optnodes = new UINode[] {
            ...
            new DynamicOptionNodeLR<int>("Boss", AssignBoss, () =>
                    key.type == 1 ?
                        campaigns[cmpIndex].bosses.Select((b, i) => (b.boss.BossPracticeName.ValueOrEn, i)).ToArray() :
