...
using Velocity = Danmaku.Velocity;
...
public static class NoExprMath_1 {
    ...
    public static LVTP LVTP(VTP vtp) {
        var dummyVel = Velocity.None;
        return (in LaserVelocity vel, in float dT, in float lT, ParametricInfo bpi, out Vector2 nrv) => {
            LaserTime = lT;
    ...
    public static BPY Smooth(string smoother, BPY x) {
        var func = EaseHelpers.GetFuncOrRemoteOrLinear(smoother);
        return b => func(x(b));
    }
    public static BPY LerpSmooth(string smoother, BPY z, BPY o, BPY c, BPY f1, BPY f2) {
        var sf = EaseHelpers.GetFuncOrRemoteOrLinear(smoother);
        return b => {
            var _z = z(b);
    ...
    public static TP LPlayer() => _ => LocationService.GetEnemyVisiblePlayer();
