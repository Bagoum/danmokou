...
public static class PrivateDataHoisting {
    ...
    private static void UploadAddOne(Reflector.ExType ext, string varName, GenCtx gcx, uint id) {
        ...
        if      (ext == Reflector.ExType.Float) 
            fData.SetDefaultSet(id, varId, gcx.GetFloatOrThrow(varName));
        else if (ext == Reflector.ExType.V2) 
            v2Data.SetDefaultSet(id, varId, gcx.V2s.GetOrThrow(varName, "GCX V2 values"));
        else if (ext == Reflector.ExType.V3) 
            v3Data.SetDefaultSet(id, varId, gcx.V3s.GetOrThrow(varName, "GCX V3 values"));
        else if (ext == Reflector.ExType.RV2) 
            rv2Data.SetDefaultSet(id, varId, gcx.RV2s.GetOrThrow(varName, "GCX RV2 values"));
