...
public class PlayerInput : BehaviorEntity {
    ...
    protected override void Awake() {
        ...
        if (LoadPlayer()) {
            ...
            MarisaATracking.Clear();
            MarisaATracking.Add(hitbox.location);
        }
    }
    ...
    private void LoadShot() {
        ...
        if (shot != null) {
            var obj = shot.playerChild ? GameObject.Instantiate(shot.prefab, tr) : GameObject.Instantiate(shot.prefab);
            var ac = obj.GetComponent<AyaCamera>();
            if (ac != null) ac.Initialize(this);
        }
    }
    ...
    public static readonly Expression firingTime = Expression.Property(null, 
        typeof(PlayerInput).GetProperty("FiringTime"));
    ...
    public static readonly Expression unfiringTime = Expression.Property(null, 
        typeof(PlayerInput).GetProperty("UnFiringTime"));

    private static readonly CircularList<Vector2> MarisaATracking = new CircularList<Vector2>(120);
    public static Vector2 MarisaAPosition(float timeAgo) =>
        MarisaATracking.SafeIndexFromBack((int) (timeAgo * ETime.ENGINEFPS));
    ...
    private void MovementUpdate(float dT, out float horiz_input, out float vert_input) {
        ...
        velocity *= AyaCamera.CameraSpeedMultiplier;
        //Check bounds
        ...
        bpi.loc = tr.position = hitbox.location = pos + velocity * dT; 
        ...
        if (IsMoving) {
            if (IsFocus) {
                ...
                for (int ii = 0; ii < MarisaATracking.arr.Length; ++ii) {
                    MarisaATracking.arr[ii] += delta;
                }
            } else {
                MarisaATracking.Add(hitbox.location);
            }
