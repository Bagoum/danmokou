...
public class Tutorial : BehaviorEntity {
    ...
    private IEnumerator RunTutorial(int skips) {
        ...
        Prompt(text00, "Resolution option ------------>\nTry changing the resolution. It takes effect immediately.", 1.6f);
        var r = SaveData.s.Resolution;
        yield return waitlf(() => SaveData.s.Resolution != r);
        Message(text00,"Refresh rate option --------------->\nThis is the game speed. The engine will determine this automatically, but you can adjust it if the game is too fast or too slow.", 0.4f);
        yield return confirm();
        Message(text00,"Fullscreen option --------------->\nSome computers have trouble playing games in fullscreen. Try turning this off if you have lag.", 0.6f);
        yield return confirm();
        ...
        BulletManager.RequestLaser(null, "mulaser-blue/b", new Velocity(new Vector2(2, 5), -90), 0, 5, 999, 0, ref nrx);
        BulletManager.RequestLaser(null, "zonelaser-green/b", new Velocity(new Vector2(3, 5), -90), 0, 5, 999, 0, ref nrx);
        ...
        yield return confirm();
        Message(text10, "The white line at the bottom of the playable area changes into a colored boss healthbar when a boss is active.");
        yield return confirm();
        yield return shift();
        Prompt(text10, "The boss has started a nonspell card. Try shooting down the boss. You do up to 25% more damage when closer to the boss.");
        yield return phase();
        ...
        yield return phase();
        Prompt(text10, @"The boss has started a survival card. You cannot shoot down the boss. Wait for the timeout to the right of this text to hit zero.");
        yield return phase();
        Message(text10, "The amount of items dropped by the boss decreases gradually after 50% of the timeout has elapsed. Defeat the boss within the first 50% of the timeout for maximum rewards. (Does not apply to survival cards.)");
        yield return confirm();
...
Fulfill the requirement to get an extra life.", 0.5f);
        ...
        yield return shift();
        Message(text10, $"Value items (blue) increase your score by {CampaignData.valueItemPoints}, with a bonus if you collect them higher on the screen or while using your special ability.");
        yield return confirm();
        yield return shift();
        Prompt(text10, "Get 75,000 points by collecting value items.");
        yield return waitlf(() => campaign.Score > 75000);
        yield return shift();
...
It will empty over time, but graze and point++ items will restore it. When empty, your multiplier will fall by {CampaignData.pivFallStep}.");
        yield return confirm();
        Message(text00, $@"While the faith meter is blue, it will not decay. Completing stage or boss sections, or collecting graze or point++ items, will add blue to the faith meter.");
        yield return confirm();
        ...
        yield return waitlf(() => campaign.Score > 2000000);
        yield return shift();
        ...
        GameManagement.campaign.AddGems(100);
        yield return waitlf(() => InputManager.IsMeter);
        yield return shift();
        Message(text10, "While in bullet time, value items and point++ items are worth twice as much, and the player moves faster. Try collecting some items with or without bullet time.");
        yield return confirm();
