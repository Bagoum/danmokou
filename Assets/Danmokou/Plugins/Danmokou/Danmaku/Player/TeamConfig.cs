...
public readonly struct TeamConfig {
    ...
    public TeamConfig(int which, Subshot sub, params (ShipConfig, ShotConfig)[] ships) {
        this.ships = ships;
        selectedIndex = which;
        subshot = sub;
    }
    public TeamConfig(Saveable saved) : this(saved.SelectedIndex, saved.Subshot, saved.Players.Select(p => (
