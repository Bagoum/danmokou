...
/// <summary>
/// A group of arbitrarily positioned nodes with an special "unselector" node that is used to remove selection from any other node.
/// <br/>Used with <see cref="XMLDynamicMenu"/>.
...
class UIFreeformGroup : UIGroup {
    ...
    private UIResult _FindClosest(UINode src, UICommand dir) {
        ...
        if (result is UIResult.GoToNode g && g.Target == unselector)
            return SilentNoOp;
        return result;
    }
