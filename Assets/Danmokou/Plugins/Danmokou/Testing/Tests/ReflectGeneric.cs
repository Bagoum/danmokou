...
namespace Danmokou.Testing {
    ...
    public static class ReflectGeneric {
        ...
        public static void TGenericCompile() {
            var func = PrepareDelegate<Func<float, float, float, Vector3>>(
            ...
            Assert.Throws<CompileException>(() => PrepareDelegate<Func<float, float>>("&doesNotExist",
                new DelegateArg<float>("myVar1")));
            Assert.Throws<BadTypeException>(() => PrepareDelegate<Func<float, Vector2, float>>("&myVec",
