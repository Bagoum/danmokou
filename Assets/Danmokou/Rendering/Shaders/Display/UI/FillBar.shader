    ...
    SubShader{
        ...
        Pass {
            ...
            float4 frag(fragment f) : SV_Target {
                ...
                float4 fc = _CE;
                fc = lerp(fc, _CF2, smoothstep(noise-0.15, noise, _F - grad + lerp(0.15, 0.2, _F)));
                fc = lerp(fc, _CF, smoothstep(noise-0.02, noise, _F - grad));
                if (_FI > 0) {
                    fc = lerp(fc, _CFI, smoothstep(noise-0.05, noise, _F * _FI - grad));
                }
            #else
                float4 fc = lerp(_CE, _CF, smoothstep(-0.01, 0.01, _F - f.uv.x));
                fc = lerp(fc, _CFI, smoothstep(-0.01, 0.01, _F * _FI - f.uv.x));
            #endif
